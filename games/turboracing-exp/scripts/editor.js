...
import { state, camEditor, editorMouse, editorCam } from './state.js';
...
import {
  getAllTracks, getTrackById, cssToHexNum, deepClone,
  makeTimeOfDayPreset, makeEditableTrackFromGameTrack
...
const CUSTOM_TRACKS_TABLE='turboracing_exp_custom_tracks';
...
export function loadEditorTracks(){
  try{
    const parsed=JSON.parse(localStorage.getItem('turboracing_exp_custom_tracks')||'[]');
...
export function persistEditorTracks(){ localStorage.setItem('turboracing_exp_custom_tracks',JSON.stringify(state.editorTracks)); }
...
export async function showTrackEditor(){
