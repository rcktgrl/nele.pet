function startGame(){syncSettingsUI();Object.assign(game,{running:true,money:180,score:0,runEarnedScore:0,runScorePenaltyMult:0,lives:20,wave:0,maxWaves:mapConfig.waveLimit,selectedTowerType:null,sellMode:false,enemies:[],towers:[],projectiles:[],effects:[],spawnQueue:[],timeUntilNextSpawn:0,intermission:true,intermissionTimer:0,waveInProgress:false,gameOver:false,victory:false,hoveredCell:null,statusTimer:0,fastSpawnAccumulator:0,pendingWaveDefs:[],currentWaveInfo:null,currentEnemyMultiplier:1,autoWaveStart:false,activeCards:getActiveLoadoutCardIds()});generateMap();for(let i=1;i<=Math.min(3,game.maxWaves);i++)game.pendingWaveDefs.push(generateWavePlan(i));buildTowerButtons();updateHUD();updateWavePreviewUI();updateSelectedTowerStats();updateStartWaveButton();setStatus('Run gestartet. Drücke Start Wave, um die erste Welle zu beginnen.');showScreen('gameScreen')}
...
function update(dt) {
  ...
  if (!game.gameOver && !game.victory) {
    for (const t of game.towers) {
      ...
      if (Math.hypot(target.x - t.x, target.y - t.y) <= t.range) {
